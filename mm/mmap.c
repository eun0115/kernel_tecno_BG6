...
{
	write_seqlock(container_of_gki(mm, struct mm_struct, seqlock_t, mm_seq));
}
static inline void mm_write_sequnlock(struct mm_struct *mm)
{
	write_sequnlock(container_of_gki(mm, struct mm_struct, seqlock_t, mm_seq));
}
static void __vm_rcu_put(struct rcu_head *head)
{
	...
	struct vm_area_struct *vma = container_of(head, struct vm_area_struct,
						  vm_rcu);
#else
	u64 *head_temp = (u64 *)head;
	struct vm_area_struct *vma = container_of(head_temp, struct vm_area_struct,
...
static void vm_rcu_put(struct vm_area_struct *vma)
{
	...
	call_rcu(container_of_gki(vma, struct vm_area_struct, struct rcu_head,
							vm_rcu), __vm_rcu_put);
}
#else
static inline void mm_write_seqlock(struct mm_struct *mm)
...
struct vm_area_struct *find_vma_rcu(struct mm_struct *mm, unsigned long addr)
{
	...
	do {
		...
		seq = read_seqbegin(container_of_gki(mm, struct mm_struct,
								seqlock_t, mm_seq));
		...
		if (vma)
			get_vma(vma);
		rcu_read_unlock();
	} while (read_seqretry(container_of_gki(mm, struct mm_struct,
									seqlock_t, mm_seq), seq));
